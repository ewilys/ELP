...
class SudokuSolver {
	...
	for (int i=0;i<9;i++){
		for(int j=0;j<9;j++){
			...
			List<Square> friendsOfS=new ArrayList<Square>();
			friendsOfS.addAll(s.getRow().squares());
			friendsOfS.remove(s);
			friendsOfS.addAll(s.getBox().squares());
			friendsOfS.remove(s);
			friendsOfS.addAll(s.getCol().squares());
			friendsOfS.remove(s);
			friends.put(s,friendsOfS);
		}
	}
	...
	while(isOk && it.hasNext()){
		...
		isOk=assign(workingGrid,marks,e.getKey(),e.getValue());
	}
	return isOk;
    }
